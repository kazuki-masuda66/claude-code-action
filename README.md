...
jobs:
  claude-response:
    ...
    steps:
      - uses: kazuki-masuda66/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: kazuki-masuda66/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`kazuki-masuda66/claude-code-base-action`](https://github.com/kazuki-masuda66/claude-code-base-action).
